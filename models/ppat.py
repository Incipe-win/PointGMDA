...
class PointPatchTransformer(nn.Module):
    def __init__(
        ...
        self.sa = PointNetSetAbstraction(
            npoint=patches, radius=prad, nsample=nsamp, in_channel=in_dim + 3, mlp=[64, 64, sa_dim], group_all=False
        )
        self.lift = nn.Sequential(
            nn.Conv1d(sa_dim + 3, dim, 1), rst.Lambda(lambda x: torch.permute(x, [0, 2, 1])), nn.LayerNorm([dim])
        )
...
class Projected(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor, device=None, quantization_size=0.05):
        return self.proj(self.ppat(xyz.transpose(-1, -2).contiguous(), features.transpose(-1, -2).contiguous()))
...
def make(scaling):
    ...
    if scaling == 1:
        return Projected(
            PointPatchTransformer(256, 6, 4, 1024, 96, 64, 0.4, 256, in_channel),
            nn.Linear(256, out_channel),
        )
    if scaling == 2:
        return Projected(
            PointPatchTransformer(512, 6, 8, 1024, 128, 64, 0.4, 256, in_channel),
            nn.Linear(512, out_channel),
        )
    if scaling == 3:
        return Projected(
            PointPatchTransformer(512, 12, 8, 1024, 128, 128, 0.35, 128, in_channel),
            nn.Linear(512, out_channel),
        )
    if scaling == 4:
        return Projected(
            PointPatchTransformer(512, 12, 8, 512 * 3, 256, 384, 0.2, 64, in_channel),
            nn.Linear(512, out_channel),
        )
    if scaling == 5:
        return Projected(
            PointPatchTransformer(768, 12, 12, 768 * 3, 256, 512, 0.2, 64, in_channel),
            nn.Linear(768, out_channel),
        )
    if scaling == 6:
        return Projected(
            PointPatchTransformer(768, 24, 12, 768 * 4, 256, 512, 0.2, 64, in_channel),
            nn.Linear(768, out_channel),
        )
