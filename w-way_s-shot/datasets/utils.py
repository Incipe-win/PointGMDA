...
class DatasetWrapper(TorchDataset):
    def __init__(self, data_source, input_size, transform=None, is_train=False, k_tfm=1):
        ...
        if self.transform:
            if isinstance(self.transform, (list, tuple)):
                for i, tfm in enumerate(self.transform):
                    pc = self._transform_pc(tfm, output["impath"])
                    keyname = "pc"
                    if (i + 1) > 1:
                        keyname += str(i + 1)
                    output[keyname] = pc
            else:
                pc = self._transform_pc(self.transform, output["impath"])
                pc = self._normalize_pointcloud(pc)
                output["pc"] = pc
        else:
            pc = self._normalize_pointcloud(output["impath"])
            output["pc"] = pc
        return output["pc"], output["label"], output["rgb"]

    def _transform_pc(self, tfm, pc0):
        img_list = []

        for k in range(self.k_tfm):
            img_list.append(tfm(pc0))
