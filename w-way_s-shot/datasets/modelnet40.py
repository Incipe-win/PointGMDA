...
class ModelNet40(DatasetBase):
    ...
    def __init__(self, root, num_shots, selected_classes=None):
    ...
    def _read_hdf5_data(self, file_path: str) -> List[Datum]:
        ...
        with h5py.File(file_path, "r") as f:
            ...
            for i in range(len(points)):
                ...
                # point_cloud = torch.from_numpy(points[i][:1024]).float()
                ...
                if self.selected_classes is not None and classname not in self.selected_classes:
                    continue

                new_label = self.class_to_idx[classname]

                data.append(Datum(impath=point_cloud, label=new_label, classname=classname, rgb=rgb))
        return data
