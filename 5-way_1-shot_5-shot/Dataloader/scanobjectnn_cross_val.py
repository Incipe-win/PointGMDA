...
class ScanObjectNN_fs(Dataset):
    ...
    def __getitem__(self, index):
        point = np.loadtxt(self.point_path[index]).astype(np.float32)[: self.num_point]
        point[:, [1, 2]] = point[:, [2, 1]]
...
def get_sets(data_path, fold=0, k_way=5, n_shot=1, query_num=15, data_aug=True):
    ...
    val_dataset = ScanObjectNN_fs(root=data_path, split="test", fold=fold, data_aug=data_aug)
...
if __name__ == "__main__":
    ...
    test_dataset, test_loader = get_sets(data_path=data_path)
